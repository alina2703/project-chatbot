...
function ConvState(wrapper, SingleConvState, form, params) {
    ...
    this.cur = SingleConvState;
    this.answers = {};
    this.parameters = params;
    this.scrollDown = function() {
        $(this.wrapper).find('#messages').stop().animate({scrollTop: $(this.wrapper).find('#messages')[0].scrollHeight}, 555);
    }.bind(this);
};
...
ConvState.prototype.next = function(){
    if(this.cur.input.hasOwnProperty('callback')) {
        window[this.cur.input.callback](this);
    }
    if(this.cur.hasNext()){
        this.cur = this.cur.next;
        if(this.cur.input.hasOwnProperty('fork') && this.cur.input.hasOwnProperty('case')){
            if(this.answers.hasOwnProperty(this.cur.input.fork) && this.answers[this.cur.input.fork].value != this.cur.input.case) {
                return this.next();
            }
            if(!this.answers.hasOwnProperty(this.cur.input.fork)) {
                return this.next();
            }
        }
        return true;
...
ConvState.prototype.printQuestion = function(){
    var questions = this.cur.input.questions;
    ...
    setTimeout(function(){
        ...
        if(this.cur.input.type=="select"){
            this.printAnswers(this.cur.input.answers, this.cur.input.multiple);
        }
        this.scrollDown();
        if(this.cur.input.hasOwnProperty('noAnswer') && this.cur.input.noAnswer===true) {
            if(this.next()){
                setTimeout(function(){
            ...
            if(answers.hasOwnProperty(i)){
                var option = $('<div class="option">'+answers[i].text+'</div>')
                    ...
                    .click(function(event){
                        var indexOf = this.cur.input.selected.indexOf($(event.target).data("answer").value);
                        if(indexOf == -1){
                            this.cur.input.selected.push($(event.target).data("answer").value);
                            $(event.target).addClass('selected');
                        } else {
                            this.cur.input.selected.splice(indexOf, 1);
                        ...
                        if(this.cur.input.selected.length > 0) {
        ...
        for(var i in answers){
            if(answers.hasOwnProperty(i)){
                var option = $('<div class="option">'+answers[i].text+'</div>')
                    .data("answer", answers[i])
                    .click(function(event){
                        this.cur.input.selected = $(event.target).data("answer").value;
                        this.wrapper.find(this.parameters.inputIdHashTagName).removeClass('error');
                        this.wrapper.find(this.parameters.inputIdHashTagName).val('');
...
ConvState.prototype.answerWith = function(answerText, answerObject) {
    ...
    if(this.cur.input.hasOwnProperty('name')){
        if(typeof answerObject == 'string') {
            if(this.cur.input.type == 'tel')
                answerObject = answerObject.replace(/\s|\(|\)|-/g, "");
            this.answers[this.cur.input.name] = {text: answerText, value: answerObject};
            this.cur.answer = {text: answerText, value: answerObject};
            //console.log('previous answer: ', answerObject);
        } else {
            this.answers[this.cur.input.name] = answerObject;
            this.cur.answer = answerObject;
        }
        if(this.cur.input.type == 'select' && !this.cur.input.multiple) {
            $(this.cur.input.element).val(answerObject.value).change();
        } else {
            $(this.cur.input.element).val(answerObject).change();
        }
    }
    //prints answer within messages wrapper
    if(this.cur.input.type == 'password')
    ...
    $(this.form).append(this.cur.input.element);
    var messageObj = $('<div class="message to typing"><div class="typing_loader"></div></div>');
    setTimeout(function(){
        $(this.wrapper).find('#messages').append(messageObj);
        this.scrollDown();
    ...
    $.fn.convform = function(options){
        ...
        */
        ...
        if(inputs.length) {
            ...
            //creates new wrapper state with first singlestate as cur and gives access to wrapper element
            ...
            $(inputForm).find(parameters.inputIdHashTagName).keypress(function(e){
                if(e.which == 13) {
                    var input = $(this).val();
                    e.preventDefault();
                    if(state.cur.input.type=="select" && !state.cur.input.multiple){
                        if(state.cur.input.required) {
                            state.wrapper.find('#userInputBot').addClass('error');
                        } else {
                            var results = state.cur.input.answers.filter(function (el) {
                                return el.text.toLowerCase().indexOf(input.toLowerCase()) != -1;
                            });
                            if (results.length) {
                                state.cur.input.selected = results[0];
                                $(this).parent('form').submit();
                            } else {
                                state.wrapper.find(parameters.inputIdHashTagName).addClass('error');
                            }
                        }
                    } else if(state.cur.input.type=="select" && state.cur.input.multiple) {
                        if(input.trim() != "") {
                            var results = state.cur.input.answers.filter(function(el){
                                return el.text.toLowerCase().indexOf(input.toLowerCase()) != -1;
                            });
                            if(results.length){
                                if(state.cur.input.selected.indexOf(results[0].value) == -1){
                                    state.cur.input.selected.push(results[0].value);
                            ...
                            if(state.cur.input.selected.length) {
                                $(this).parent('form').submit();
                            }
                        }
                    } else {
                ...
                if(state.cur.input.type=="select"){
                    var input = $(this).val();
                    var results = state.cur.input.answers.filter(function(el){
                        return el.text.toLowerCase().indexOf(input.toLowerCase()) != -1;
                    });
                    if(results.length){
                        ...
                        state.printAnswers(results, state.cur.input.multiple);
                    } else {
                        state.wrapper.find(parameters.inputIdHashTagName).addClass('error');
                    }
                } else if(state.cur.input.hasOwnProperty('pattern')) {
                    var reg = new RegExp(state.cur.input.pattern, 'i');
            ...
            $(inputForm).submit(function(e){
                ...
                if(state.cur.input.type == 'select'){
                    if(!state.cur.input.multiple){
                        state.answerWith(state.cur.input.selected.text, state.cur.input.selected);
                    } else {
                        state.answerWith(state.cur.input.selected.join(', '), state.cur.input.selected);
                    }
                } else {
